...
module.exports.sockets = {
    ...
    beforeConnect: function (handshake, cb) {
        ...
        sails.log.info('beforeConnect');
        var availableRooms = JSON.stringify(sails.sockets.rooms());
        sails.log.info('availableRooms -beforeConnect' + availableRooms);
        return cb(null, true);
    ...
    // },
    afterDisconnect: function (session, socket, cb) {
        sails.log.info('afterDisconnect');
        if (session !== undefined && socket !== undefined) {
            sails.log.info('afterDisconnect with socket');
            var roomNames = JSON.stringify(sails.sockets.socketRooms(socket));
            sails.log.info('Socket Rooms -Disconnect' + roomNames);
            var availableRooms = JSON.stringify(sails.sockets.rooms());
            sails.log.info('availableRooms -Disconnect' + availableRooms);
            sails.log.info('session.operatorId -Disconnect' + JSON.stringify(session));
            if (session.operatorId !== undefined) {
                sails.sockets.subscribers(session.operatorId, function (err, socket_ids) {
